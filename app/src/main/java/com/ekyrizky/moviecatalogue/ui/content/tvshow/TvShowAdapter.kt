...
class TvShowAdapter : RecyclerView.Adapter<TvShowAdapter.ListViewHolder>() {
    private  var listTvShows = mutableListOf<TvShowResponse>()

    fun setItems(tvShowItems: MutableList<TvShowResponse>) {
        this.listTvShows.clear()
        this.listTvShows.addAll(tvShowItems)
    ...
    inner class ListViewHolder(private val binding: ItemsGridBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(tvShowItems: TvShowResponse) {
            val imgSize = "w500"
            val imgUrl = BuildConfig.BASE_IMG
            with(binding) {
                tvTitle.text = tvShowItems.originalName
                tvReleaseYear.text = dateConverter(tvShowItems.firstAirDate.toString())
                Glide.with(itemView.context)
                        .load("$imgUrl$imgSize${listTvShows[adapterPosition].posterPath}")
                        .apply(RequestOptions.placeholderOf(R.drawable.ic_loading)
                                .error(R.drawable.ic_error))
                        .into(imgPoster)
                cardContent.setOnClickListener {
                    Intent(it.context, DetailActivity::class.java).apply {
                        putExtra(DetailActivity.EXTRA_CONTENT, tvShowItems.id)
                        putExtra(DetailActivity.EXTRA_TYPE, DetailActivity.EXTRA_TVSHOW)
                    }.also { intent ->
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) = holder.bind(listTvShows[position])
